<template>
    <div
        ...
        <div
            ...
            <div class="flex h-full items-center gap-[2px]" v-show="statusShowTitle">
                <button @click="ShowTestCase = true"
                    ...
                    <span
                        :class="`text-[16px] ${ShowTestCase ? 'text-[#000000] dark:text-[#E6E6E6]  font-medium' : 'text-[#6B6B6B] dark:text-[#C8C8C8] font-light'} `">
                        Test Case</span>
                </button>
                ...
                <button @click="ShowTestCase = false"
                    ...
                    <font-awesome-icon :icon="['fas', 'terminal']"
                        :class="`text-[20px] ${!ShowTestCase ? 'text-[#00C7A3]' : 'text-[#7FDBC1]'}`" />
                    <span
                        :class="`text-[16px] ${!ShowTestCase ? 'text-[#000000] dark:text-[#E6E6E6]  font-medium' : 'text-[#6B6B6B] dark:text-[#C8C8C8] font-light'} `">Test
                        Result</span>
                </button>
            </div>
        ...
        <div class="flex-auto w-full h-[calc(100%-36px)] p-3 overflow-y-auto" v-if="ShowTestCase">
            ...
            <div class="flex flex-col gap-y-3 min-w-full">
                <div class="flex items-center w-full gap-x-3"
                    v-for="(item, index) in TestCaseList[TestCaseSelectIndex].input">
                    <p class="text-[16px] text-[#202020] dark:text-[#E6E6E6] flex-none leading-4">Input line</p>
                    <input type="text" v-model="TestCaseList[TestCaseSelectIndex].input[index].input"
                        @change="updateInput" @input="updateInput"
                    ...
                    <button @click="removeInput(TestCaseSelectIndex, index)"
                        class="text-[16px] text-[#FEFEFE] h-8 w-8 rounded-md bg-red-500 hover:bg-red-600 flex items-center justify-center">
                        <font-awesome-icon :icon="['fas', 'trash']" />
                    </button>
                </div>
                <div class="flex items-center w-full gap-x-3">
                    <p class="text-[16px] text-[#202020] dark:text-[#E6E6E6] flex-none leading-4">Input Add </p>
                    <button @click="addInput(TestCaseSelectIndex)"
                        class="flex-auto bg-[#606060] dark:bg-[#FEFEFE] hover:bg-[#303030] dark:hover:bg-[#5b5b5b] active:bg-[#303030] active:hover:bg-[#5b5b5b] drop-shadow-md rounded-md py-[6px] px-3 text-[16px] font-medium text-[#FEFEFE] dark:text-[#202020] dark:hover:text-[#FEFEFE] dark:active:text-[#FEFEFE]">
                        เพิ่ม input
                    </button>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-y-3 min-w-full">
                <div class="flex items-center w-full gap-x-3">
                    <p class="text-[16px] text-[#202020] dark:text-[#E6E6E6] flex-none leading-4">Target
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>
                    <input type="text" v-model="TestCaseList[TestCaseSelectIndex].target"
        ...
        <!-- Test Result -->
        <div v-else-if="ShowTestCase == false && resultTest == null && testLoading == false && resultType != 'submit'"
            class="flex-auto">
            <div class="min-w-full min-h-full flex items-center justify-center">
                <p class="text-[20px] font-light text-[#303030] dark:text-[#FEFEFE]">คุณควร รัน โปรแกรม ที่คุณเขียนก่อน
                </p>
            </div>
        </div>
            ...
            <div class="min-w-full min-h-full flex items-center justify-center">
                <div class="flex gap-x-3 items-center">
                    <div class="mx-auto py-[1.154px] px-1 w-fit h-fit">
                        <svg class=" animate-spin -ml-1 h-[28px] w-[28px] text-[#303030] dark:text-[#FEFEFE]"
        ...
        <div v-else class="flex-auto w-full h-[calc(100%-36px)] overflow-y-auto">
            ...
            <div class="flex flex-col gap-y-2 p-3" v-if="resultType == 'test' && resultTest != null">
                ...
                <!-- ถ้า code error -->
                <div v-if="resultTest.status == 'code error'"
                    class="w-full h-fit px-2 py-1 rounded-md bg-[#ffe5e5] text-[16px] text-red-500 my-3 ">{{
                    resultTest.code_error }}</div>
                ...
                <div class="w-full rounded bg-gray-200 px-2 py-1">
                    <p v-if="TestCaseList[TestCaseSelectIndex].target != ''" class="text-[16px]">{{
                        TestCaseList[TestCaseSelectIndex].target }}</p>
                    <p v-else class="text-[16px]">&nbsp;</p>
                </div>
            </div>
            ...
            <div class="w-full h-full" v-else-if="resultType == 'submit' && TestCaseCount != 0">
                ...
                <!-- รายการ Testcase ที่ทดสอบ มี ai check -->
                <div class="flex flex-col px-3 divide-y divide-gray-300" v-else-if="props.llmCheck && resultSubmit[0] != undefined && resultSubmit[0].status == 'success_llm'">
                    <div class="w-full flex flex-col" v-for="i in TestCaseCount">
                        <div class="w-full flex justify-between" :class="ShowTestResult[i] ? 'py-5' : 'py-5'">
                            <p class="text-[24px] leading-5">Test Case {{ i }}</p>

                            <div v-if="resultSubmit[i] == undefined" class="py-[1.154px] px-1 w-fit h-fit">
                                <svg class=" animate-spin -ml-1 h-[20px] w-[20px] text-[#303030] dark:text-[#FEFEFE]"
                                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            ...
                            <div v-else class="flex items-center gap-x-2">
                                <p v-if="resultSubmit[i].status == 'success'"class="text-[18px] leading-5 text-green-500 capitalize">{{ resultSubmit[i].status}}</p>
                                <p v-else class="text-[18px] leading-5 text-red-500 capitalize">{{
                                    resultSubmit[i].status }}</p>
                                <p class="text-[20px] text-[#5e5d5d] leading-3"
                                    @click="ShowTestResult[i] = !ShowTestResult[i]">
                                    <font-awesome-icon :icon="['fas', 'chevron-down']" class="transition duration-500"
                                        :class="ShowTestResult[i] ? 'rotate-180' : 'rotate-0'" />
                                </p>
                            </div>
                        </div>
                        <div class="w-full flex flex-col gap-y-3 mb-5" v-if="resultSubmit[i] != undefined && ShowTestResult[i]">
                            <p v-if="resultSubmit[i].status == 'success'" class="text-[22px] leading-5 text-green-500">Success</p>
                            <p v-else class="text-[22px] leading-7 text-red-500 capitalize">{{ resultSubmit[i].status}}</p>

                            <!-- ถ้า code error -->
                            <div v-if="resultSubmit[i].status == 'code error'" class="w-full h-fit px-2 py-1 rounded-md bg-[#ffe5e5] text-[16px] text-red-500 my-3 ">
                                {{resultSubmit[i].code_error }}
                            </div>

                            <div>
                                <p class="text-[16px] leading-4">Input</p>
                                <div class="w-full rounded bg-gray-200 px-2 py-1 mt-1">
                                    <p v-if="resultSubmit[i].input != ''" class="text-[16px]">{{
                                        resultSubmit[i].input }}</p>
                                    <p v-else class="text-[16px]">&nbsp;</p>
                                </div>
                            </div>
                ...
                <div class="flex flex-col px-3 divide-y divide-gray-300" v-else>
                    <div class="w-full flex flex-col" v-for="i in TestCaseCount">
                        <div class="w-full flex justify-between" :class="ShowTestResult[i] ? 'py-5' : 'py-5'">
                            <p class="text-[24px] leading-5">Test Case {{ i }}</p>
                            ...
                            <div v-else class="flex items-center gap-x-2">
                                ...
                                <p v-else class="text-[18px] leading-5 text-red-500 capitalize">{{
                                    resultSubmit[i-1].status }}</p>
                                <p class="text-[20px] text-[#5e5d5d] leading-3"
                                    @click="ShowTestResult[i - 1] = !ShowTestResult[i - 1]">
                                    <font-awesome-icon :icon="['fas', 'chevron-down']" class="transition duration-500"
                                        :class="ShowTestResult[i - 1] ? 'rotate-180' : 'rotate-0'" />
                                </p>
                            </div>
                        </div>
                        <div class="w-full flex flex-col gap-y-3 mb-5" v-if="resultSubmit[i - 1] != undefined && ShowTestResult[i - 1]">
                            ...
                            <!-- ถ้า code error -->
                            <div v-if="resultSubmit[i - 1].status == 'code error'" class="w-full h-fit px-2 py-1 rounded-md bg-[#ffe5e5] text-[16px] text-red-500 my-3 ">
                                {{resultSubmit[i-1].code_error }}
                            </div>

                            <div>
                                <p class="text-[16px] leading-4">Input</p>
                                <div class="w-full rounded bg-gray-200 px-2 py-1 mt-1">
                                    <p v-if="resultSubmit[i - 1].input != ''" class="text-[16px]">{{
                                        resultSubmit[i-1].input }}</p>
                                    <p v-else class="text-[16px]">&nbsp;</p>
                                </div>
                            </div>
                            ...
                            <div v-if="resultSubmit[i - 1].status != 'code error'">
                                <p class="text-[16px] leading-4">Output</p>
                                <div class="w-full rounded bg-gray-200 px-2 py-1 mt-1">
                                    <p v-if="resultSubmit[i - 1].code_output != ''" class="text-[16px]">
                                        {{resultSubmit[i-1].code_output }}
                                    </p>
                                    <p v-else class="text-[16px]">&nbsp;</p>
                                </div>
                            </div>
